...
class _HistoriqueVentesState extends State<HistoriqueVentes> {
            ...
            onPressed: () async {
              if (nomController.text.isNotEmpty) {
                ...
                ScaffoldMessenger.of(
                  context,
                ).showSnackBar(const SnackBar(content: Text("Magasin créé")));
              }
            },
  ...
  void _imprimerHistorique() async {
    ...
    await PdfService.imprimerJournalVentes(
      filteredList,
      _dateFiltre.isEmpty ? "Global" : _dateFiltre,
    );
  }
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              "Date: ${vente['date_vente'].toString().substring(0, 16).replaceAll('T', ' ')}",
            ),
            ...
            ListTile(
              ...
              onTap: () async {
                ...
                try {
                  ...
                  isSuccess = await ApiService().createConfigurationFile(
                    config,
                  );
                } catch (erreur) {
              ...
              child: DropdownButtonFormField<int?>(
                ...
                items: [
                  const DropdownMenuItem(
                    value: null,
                    child: Text("Global (Mon Entreprise)"),
                  ),
                  ..._allDepots.map(
                    (d) => DropdownMenuItem(
                      value: d['idDepot'],
          ...
          Expanded(
            child: FutureBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                final filteredList = rawList.where((vente) {
                  final nomP = (vente['nom_produit'] ?? "")
                      .toString()
                      .toLowerCase();
                  final nomC = (vente['nom_client'] ?? "")
                      .toString()
                      .toLowerCase();
                  final q = _searchQuery.toLowerCase();
                  return nomP.contains(q) || nomC.contains(q);
                ...
                double total = filteredList.fold(
                  0,
                  (sum, item) =>
                      sum + ((item['prix_total'] as num?)?.toDouble() ?? 0.0),
                );
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return Card(
                                  ...
                                  child: ListTile(
                                    ...
                                    title: Text(
                                      vente['nom_produit'] ?? "Produit",
                                      style: const TextStyle(
                                        fontWeight: FontWeight.bold,
                                      ),
                                    ),
                                    subtitle: Text(
                                      "${vente['nom_client'] ?? 'Divers'} - ${vente['date_vente'].toString().substring(0, 10)}",
                                    ),
                                    onTap: () => _afficherFacture(vente),
                                    trailing: Text(
                                      "${vente['prix_total']} USD",
                                      style: TextStyle(
                                        fontWeight: FontWeight.bold,
