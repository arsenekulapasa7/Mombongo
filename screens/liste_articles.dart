...
class _ListeArticlesState extends State<ListeArticles> {
  ...
  void _loadUserData() async {
    ...
    List<Map<String, dynamic>> depotsCharge = await DatabaseHelper().getDepots(
      magId,
    );
  ...
  Future<void> _verifierStatutInstallation() async {
    ...
    final statutSecure = await FlutterSecureStorage().read(
    ...
    setState(() {
      if (idx == -1) {
        ...
        if (cartItems[idx].quantity < art.quantite) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Limite de stock atteinte")),
          );
        }
      }
    });
              ...
              child: DropdownButtonFormField<int?>(
                ...
                items: [
                  const DropdownMenuItem(
                    value: null,
                    child: Text("Tous les dépôts (Vue Entreprise)"),
                  ),
                  ..._allDepots.map(
              ...
              child: FutureBuilder<List<Map<String, dynamic>>>(
                ...
                builder: (context, snapshot) {
                  ...
                  final filteredData = snapshot.data!.where((item) {
                    return item['nom'].toString().toLowerCase().contains(
