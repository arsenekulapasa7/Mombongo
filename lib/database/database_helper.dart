...
class DatabaseHelper {
  ...
  Future _onCreate(Database db, int version) async {
    await db.execute('CREATE TABLE magasins (idMagasin INTEGER PRIMARY KEY AUTOINCREMENT, nomMagasin TEXT NOT NULL UNIQUE)');
    await db.execute('CREATE TABLE depots (idDepot INTEGER PRIMARY KEY AUTOINCREMENT, nomDepot TEXT NOT NULL, magasin_id INTEGER NOT NULL, FOREIGN KEY (magasin_id) REFERENCES magasins (idMagasin) ON DELETE CASCADE)');
    await db.execute('CREATE TABLE produits (id INTEGER PRIMARY KEY AUTOINCREMENT, nom TEXT NOT NULL COLLATE NOCASE, quantite INTEGER DEFAULT 0, prix_unitaire REAL DEFAULT 0.0, depot_id INTEGER, FOREIGN KEY (depot_id) REFERENCES depots (idDepot) ON DELETE CASCADE)');
    await db.execute('CREATE TABLE ventes (id INTEGER PRIMARY KEY AUTOINCREMENT, id_transaction TEXT, produit_id INTEGER, nom_produit TEXT, nom_client TEXT, quantite_vendue INTEGER, prix_total REAL, date_vente TEXT, depot_id INTEGER, FOREIGN KEY (depot_id) REFERENCES depots (idDepot) ON DELETE CASCADE)');
    await db.execute('CREATE TABLE mouvements (id INTEGER PRIMARY KEY AUTOINCREMENT, produit_id INTEGER, nom_produit TEXT, quantite INTEGER, type TEXT, date_mouvement TEXT, depot_id INTEGER, FOREIGN KEY (depot_id) REFERENCES depots (idDepot) ON DELETE CASCADE)');
    ...
    try {
      return await db.transaction((txn) async {
        ...
        if (resMag.isNotEmpty) {
          ...
          magasinId = await txn.insert('magasins', {'nomMagasin': nomMagasin});
        }
      ...
      if (e.toString().contains("UNIQUE constraint failed: utilisateurs.nomUser")) {
  ...
  Future<List<Map<String, dynamic>>> getUtilisateurs(int magasinId) async {
    ...
    return await db.rawQuery('''
      SELECT u.*, d.nomDepot 
      FROM utilisateurs u
      LEFT JOIN depots d ON u.depot_id = d.idDepot
      WHERE u.magasin_id = ?
    ''', [magasinId]);
  }
  ...
  Future<int> validerUtilisateur(int idUser) async {
    Database db = await database;
    return await db.update('utilisateurs', {'UserState': 1}, where: 'idUser = ?', whereArgs: [idUser]);
  }

  Future<int> changerDepotUtilisateur(int idUser, int newDepotId) async {
    Database db = await database;
    return await db.update('utilisateurs', {'depot_id': newDepotId}, where: 'idUser = ?', whereArgs: [idUser]);
  }
  ...
  Future<int> addDepot(String nom, int magasinId) async {
    Database db = await database;
    return await db.insert('depots', {'nomDepot': nom, 'magasin_id': magasinId});
  }
  ...
  Future<List<Map<String, dynamic>>> getRapportGlobal(int? depotId, {int? magasinId}) async {
    ...
    return await db.rawQuery('''SELECT nom_produit, quantite_vendue as quantite, 'VENTE' as type, date_vente as date FROM ventes WHERE $filter UNION ALL SELECT nom_produit, quantite, type, date_mouvement as date FROM mouvements WHERE $filter ORDER BY date DESC''', args);
  }
  ...
  Future<int> insertProduit(Map<String, dynamic> row, int depotId) async {
    ...
    if (existants.isNotEmpty) {
      ...
      await db.update('produits', {'prix_unitaire': prix}, where: 'id = ?', whereArgs: [idExistant]);
      return await reaprovisionner(idExistant, qte, depotId);
    } else {
      int id = await db.insert('produits', {'nom': nom, 'quantite': qte, 'prix_unitaire': prix, 'depot_id': depotId});
      await db.insert('mouvements', {'produit_id': id, 'nom_produit': nom, 'quantite': qte, 'type': 'ENTREE', 'date_mouvement': DateTime.now().toIso8601String(), 'depot_id': depotId});
      return id;
    }
  }
  ...
  Future<int> reaprovisionner(int id, int qte, int depotId) async {
    ...
    await db.insert('mouvements', {'produit_id': id, 'nom_produit': nom, 'quantite': qte, 'type': 'REAPPRO', 'date_mouvement': DateTime.now().toIso8601String(), 'depot_id': depotId});
    return await db.rawUpdate('UPDATE produits SET quantite = quantite + ? WHERE id = ? AND depot_id = ?', [qte, id, depotId]);
  }
    ...
    try {
      await db.transaction((txn) async {
        for (var item in items) {
          ...
          await txn.update('produits', {'quantite': stockActuel - item.quantity}, where: 'id = ? AND depot_id = ?', whereArgs: [item.article.id, effectiveDepotId]);
          await txn.insert('ventes', {'id_transaction': idTransaction, 'produit_id': item.article.id, 'nom_produit': item.article.nom, 'nom_client': nomClient, 'quantite_vendue': item.quantity, 'prix_total': item.article.prix * item.quantity, 'date_vente': dateVente, 'depot_id': effectiveDepotId});
        }
      });
      return true;
  ...
  Future<List<Map<String, dynamic>>> getVentesParDate(String datePrefix, int? depotId, {int? magasinId}) async {
    Database db = await database;
    if (depotId != null) {
      return await db.query('ventes', where: 'depot_id = ? AND date_vente LIKE ?', whereArgs: [depotId, '$datePrefix%'], orderBy: 'date_vente DESC');
    } else if (magasinId != null) {
      return await db.rawQuery('''SELECT v.* FROM ventes v JOIN depots d ON v.depot_id = d.idDepot WHERE d.magasin_id = ? AND v.date_vente LIKE ? ORDER BY v.date_vente DESC''', [magasinId, '$datePrefix%']);
    }
    return [];
  }
